...
function setEndPoint(toStart, node, offset, range) {
  ...
  if (toStart) { //开始 开始文本所在的标签
    range.startContainer = node;
    range.startOffset = offset;
    if (!range.endContainer) {
      range.collapse(true); //这个方法会设置collapsed=true
    }
  } else { //结束 结束文本所在的标签
  ...
  updateCollapse(range);//这个方法会设置collapsed
  return range;
}
...
Range.prototype = {
  ...
  txtToElmBoundary: function (ignoreCollapsed) {
    function adjust(r, c) {
      var container = r[c + "Container"], //startContainer endContainer
        offset = r[c + "Offset"]; //startOffset endOffset
      if (container.nodeType == 3) { // 文本节点
        if (!offset) { //第一个节点
          r[
            ...
            "Before"
          ](container); // r[setStartBefore](startContainer)
          ...
          r[
            ...
            "After"
          ](container); // r[setEndAfter](endContainer)
        }
      }
    }
  ...
  createBookmark: function (serialize, same) {
    ...
    // jing
    var textContent = this.wrap.textContent;
    var startMark = textContent.indexOf(startId);
    var endMarkInit = textContent.indexOf(endId);
    var endMark =  endMarkInit - startId.length;
    var content = textContent.substring(startMark, endMarkInit).replace(startId, '');
  ...
  applyInlineStyle: function (tagName, attrs, list, callback) {
    ...
    var bookmark = this.createBookmark(),
      ...
      // console.log(bookmark, end, current, node, pre, range )
    while (
      current &&
